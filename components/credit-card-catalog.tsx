...
import { CheckCircle2, ExternalLink, GitCompareArrows, Search, SlidersHorizontal, X } from "lucide-react"
...
const sortOptions = ["Featured", "Name A–Z", "Fee: low to high"]

function getIssuer(name: string) {
  const issuer = name.match(/^(Axis|SBI|Indusind|IDFC|BOB|Bank of Baroda)/i)?.[0]
  return issuer ?? "Partner offer"
}
    ...
    <Card className="flex h-full flex-col border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-lg">
      <CardHeader className="gap-3">
        <div className="flex items-start justify-between gap-3">
          <div className="flex flex-wrap gap-2"><Badge variant="secondary" className="bg-emerald-50 text-emerald-700">{card.category}</Badge><Badge variant="outline" className="text-slate-500">{getIssuer(card.name)}</Badge></div>
...
export function CreditCardCatalog({ compact = false }: { compact?: boolean }) {
  ...
  const issuers = useMemo(() => ["All issuers", ...Array.from(new Set(creditCardProducts.map((card) => getIssuer(card.name))))], [])
  const filteredCards = useMemo(() => {
    ...
    return creditCardProducts.filter((card) => {
      ...
      const matchesIssuer = issuer === "All issuers" || getIssuer(card.name) === issuer
      return matchesQuery && matchesCategory && matchesFee && matchesIssuer
    }).sort((a, b) => sort === "Name A–Z" ? a.name.localeCompare(b.name) : sort === "Fee: low to high" ? Number.parseInt(a.fees.replace(/[^0-9]/g, ""), 10) - Number.parseInt(b.fees.replace(/[^0-9]/g, ""), 10) : 0).slice(0, compact ? 6 : undefined)
  }, [category, compact, fee, issuer, query, sort])
  ...
  return (
    <div className="space-y-8">
      {!compact && <div className="grid gap-4 rounded-2xl border border-slate-200 bg-slate-50 p-4 md:grid-cols-[1.5fr_1fr_1fr_1fr_1fr_auto]">
        ...
        <select aria-label="Filter by issuer" value={issuer} onChange={(event) => setIssuer(event.target.value)} className="h-10 rounded-md border border-input bg-background px-3 text-sm">{issuers.map((item) => <option key={item}>{item}</option>)}</select>
        <select aria-label="Sort cards" value={sort} onChange={(event) => setSort(event.target.value)} className="h-10 rounded-md border border-input bg-background px-3 text-sm">{sortOptions.map((item) => <option key={item}>{item}</option>)}</select>
        <Button variant="outline" onClick={() => { setQuery(""); setCategory("All categories"); setFee("All fees"); setIssuer("All issuers"); setSort("Featured") }}><SlidersHorizontal className="mr-2 size-4" />Reset</Button>
      </div>}

      <div className="flex flex-wrap items-center justify-between gap-3"><p className="text-sm text-slate-600">Showing <strong>{filteredCards.length}</strong> of {creditCardProducts.length} offers{!compact && " · Select up to 3 to compare"}</p>{compact ? <Button asChild variant="outline"><Link href="/credit/affiliate-credit-cards">View all offers</Link></Button> : <Button asChild variant="outline"><Link href="/credit/affiliate-credit-cards/link-report">Check link health</Link></Button>}</div>
      ...
      {!compact && selectedCards.length > 0 && <section aria-labelledby="compare-heading" className="rounded-2xl border border-emerald-200 bg-emerald-50 p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 id="compare-heading" className="flex items-center gap-2 text-xl font-bold text-slate-950"><GitCompareArrows className="size-5 text-emerald-700" />Compare selected cards</h2><p className="mt-1 text-sm text-slate-600">Comparing {selectedCards.length} of 3 available slots.</p></div><div className="flex items-center gap-2"><Button asChild className="bg-emerald-600 hover:bg-emerald-700"><Link href={`/credit/affiliate-credit-cards/compare?offers=${selectedIds.join(",")}`}>Open comparison</Link></Button><Button variant="ghost" onClick={() => setSelectedIds([])}>Clear comparison <X className="ml-2 size-4" /></Button></div></div><div className="mt-5 grid gap-3 md:grid-cols-3">{selectedCards.map((card) => <div key={card.id} className="rounded-xl bg-white p-4"><h3 className="font-bold text-slate-950">{card.name}</h3><dl className="mt-3 space-y-2 text-sm"><div className="flex justify-between gap-3"><dt className="text-slate-500">Fee</dt><dd className="text-right font-semibold">{card.fees}</dd></div><div className="flex justify-between gap-3"><dt className="text-slate-500">Rewards</dt><dd className="text-right font-semibold">{card.cashback}</dd></div><div className="flex justify-between gap-3"><dt className="text-slate-500">Type</dt><dd className="text-right font-semibold">{card.type}</dd></div></dl></div>)}</div></section>}
    </div>
  )
}

export function CreditCardCatalogSection() {
  return <section id="credit-card-catalog" className="bg-slate-50 px-4 py-16"><div className="mx-auto max-w-6xl"><div className="mb-10 max-w-2xl"><Badge className="mb-3 bg-emerald-100 text-emerald-800">Curated card marketplace</Badge><h2 className="text-3xl font-bold tracking-tight text-slate-950 md:text-4xl">Find the right credit card for your everyday.</h2><p className="mt-3 text-slate-600">Compare available card and finance offers, then apply directly through BuyNswipe&apos;s partner links.</p><p className="mt-3 text-xs leading-5 text-slate-500">Offers are supplied by external partners. Fees, eligibility, rewards, and approval terms may change—confirm details on the partner page before applying.</p></div><CreditCardCatalog compact /></div></section>
}

export function CreditCardCatalogPage() {
  return <main className="min-h-screen bg-white"><section className="bg-slate-950 px-4 py-16 text-white md:py-24"><div className="mx-auto max-w-6xl"><Badge className="mb-4 bg-emerald-400 text-emerald-950">{creditCardProducts.length} partner offers</Badge><h1 className="max-w-3xl text-4xl font-bold tracking-tight md:text-6xl">Compare cards built around how you spend.</h1><p className="mt-5 max-w-2xl text-lg leading-8 text-slate-300">Search cashback, lifestyle, shopping, premium, and finance offers, compare up to three options, then apply directly with the issuing partner.</p><p className="mt-5 max-w-2xl text-sm leading-6 text-slate-400">BuyNswipe may receive a referral fee when you apply. Always verify current partner terms, fees, eligibility, and approval criteria before applying.</p></div></section><section className="px-4 py-12 md:py-16"><div className="mx-auto max-w-6xl"><CreditCardCatalog /></div></section></main>
}
