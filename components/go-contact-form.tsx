...
export function GoContactForm() {
  ...
  return (
    <form className="flex flex-col gap-4" onSubmit={(event) => { event.preventDefault(); setSubmitted(true) }}>
      ...
      <button type="submit" className="inline-flex items-center justify-center gap-2 rounded-full bg-go-blue px-6 py-3.5 text-sm font-bold text-primary-foreground transition hover:bg-go-navy">Start a conversation <ArrowUpRight className="size-4" aria-hidden="true" /></button>
    </form>
  )
}
