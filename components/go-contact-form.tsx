...
export function GoContactForm() {
  ...
  if (submitted) {
    return (
      <div role="status" aria-live="polite" className="flex min-h-52 flex-col items-center justify-center gap-3 rounded-2xl border border-go-green/30 bg-go-green/5 p-6 text-center">
  ...
  return (
    <form
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          if (!response.ok) throw new Error(result.error || "We could not submit your message.")
          setSubmitted(true)
        } catch (submissionError) {
          setError(submissionError instanceof Error ? submissionError.message : "We could not submit your message.")
        } finally {
    ...
    >
      ...
      <button type="submit" disabled={isSubmitting} className="inline-flex items-center justify-center gap-2 rounded-full bg-go-blue px-6 py-3.5 text-sm font-bold text-primary-foreground transition hover:bg-go-navy">
