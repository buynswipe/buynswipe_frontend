  ...
  return (
    <main className="min-h-screen bg-slate-50">
      ...
      <section className="px-4 py-12 md:py-16"><div className="mx-auto grid max-w-5xl gap-6 lg:grid-cols-[1.3fr_.7fr]">
        ...
        <aside className="h-fit rounded-3xl border border-slate-200 bg-white p-6 shadow-sm md:p-8"><div className="flex items-center gap-2 text-sm font-semibold text-emerald-700"><ShieldCheck className="size-4" /> Partner terms apply</div><dl className="mt-6 space-y-5"><div><dt className="text-sm text-slate-500">Annual fee</dt><dd className="mt-1 text-lg font-bold text-slate-950">{offer.fees}</dd></div><div><dt className="text-sm text-slate-500">Rewards / cashback</dt><dd className="mt-1 text-lg font-bold text-slate-950">{offer.cashback}</dd></div><div><dt className="text-sm text-slate-500">Offer type</dt><dd className="mt-1 text-lg font-bold text-slate-950">{offer.type}</dd></div></dl><p className="mt-7 border-t pt-5 text-xs leading-5 text-slate-500">BuyNswipe may receive a referral fee. Fees, rewards, eligibility, and approval criteria can change. Confirm the current details on the partner page before applying.</p><Button asChild className="mt-6 w-full bg-emerald-600 hover:bg-emerald-700"><a href={offer.affiliateLink} target="_blank" rel="noopener noreferrer">Apply Now <ExternalLink className="ml-2 size-4" /></a></Button></aside>
      </div></section>
    </main>
  )
}
