...
export default async function ShopMerchantsAdminPage() {
  ...
  const pendingCount = merchantRows.filter((merchant) => merchant.status === "pending_review").length
  const verifiedCount = merchantRows.filter((merchant) => merchant.verification_status === "verified").length
  const connectedCount = merchantRows.filter((merchant) => merchant.affiliate_status === "connected").length
  ...
  return (
    <main className="space-y-8">
      ...
      <section className="grid gap-4 sm:grid-cols-3" aria-label="Merchant registry metrics"><Metric label="Pending review" value={pendingCount} detail="Lifecycle queue" /><Metric label="Verified" value={verifiedCount} detail="Trust status" /><Metric label="Affiliate connected" value={connectedCount} detail="Ready for attribution" /></section>
...
function Metric({ label, value, detail }: { label: string; value: number; detail: string }) {
  return <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-[0.14em] text-slate-400">{label}</p><p className="mt-3 text-3xl font-black text-slate-950">{value}</p><p className="mt-1 text-sm text-slate-500">{detail}</p></div>
}
