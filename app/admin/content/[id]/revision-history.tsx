...
export function RevisionHistory({ document, revisions }: { document: { id: string; title: string; excerpt: string | null; content: string; seo_title: string | null; seo_description: string | null }; revisions: Revision[] }) {
  ...
  async function restoreRevision(revisionId: string) {
    if (!window.confirm("Restore this revision as a draft?")) return
    setSaving(true); setMessage("")
    const response = await fetch(`/api/admin/content/${document.id}/revisions/${revisionId}`, { method: "POST" })
    const result = await response.json()
    setMessage(response.ok ? "Revision restored as a draft." : result.error || "Unable to restore revision")
    setSaving(false)
